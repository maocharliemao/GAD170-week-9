...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float pitch = 0.0f; //mouse movment
    ...
    private bool isGrounded; // when touch ground
    ...
    private void Update()
    {
        ...
        pitch -= speedV * Input.GetAxis("Mouse Y"); // wasd and arrow keys universal

        // Get the Left/Right and Forward/Back values of the input being used (WASD, Joystick etc.)
        float x = Input.GetAxis("Horizontal"); // vector 1 ro the right - 1 to the left
        float z = Input.GetAxis("Vertical");
 
        // Let the player jump if they are on the ground and they press the jump button
        if (Input.GetButtonDown("Jump") && isGrounded) 
        {
            velocity.y = Mathf.Sqrt(jumpHeight * -2 * gravity); //only interacting with upwards
        }
        ...
        Vector3 move = transform.right * x + transform.forward * z; //left and right adding, transform from isnpector
 
        // Finally, it applies that vector it just made to the character
        controller.Move(move * speed * Time.deltaTime + velocity * Time.deltaTime); // throws vector 3 in
    }
}